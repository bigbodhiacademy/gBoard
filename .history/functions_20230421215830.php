...
function getLeaderboardData($offset, $limit) {
  global $conn;

  $sql = "SELECT students.name, leaderboard.total_points, leaderboard.average_score FROM leaderboard INNER JOIN students ON leaderboard.student_id = students.id ORDER BY total_points DESC, average_score DESC LIMIT ?, ?";
  $stmt = $conn->prepare($sql);
