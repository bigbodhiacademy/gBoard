...
<form id="leaderboardFilterForm">
  ...
  <input type="month" name="month" id="leaderboardMonth" style="display:none;">
  <input type="number" name="year" id="leaderboardYear" min="1900" max="2099" step="1" value="2023" style="display:none;">
...
document.getElementById('leaderboardFilterForm').addEventListener('submit', function (event) {
  ...
  c  const interval = document.getElementById('leaderboardInterval').value;
  const month = document.getElementById('leaderboardMonth').value;
  const year = document.getElementById('leaderboardYear').value;

  // Make an AJAX request to fetch the leaderboard data based on the selected interval
  ...
  xhr.send(`interval=${interval}&month=${month}&year=${year}`);
});
