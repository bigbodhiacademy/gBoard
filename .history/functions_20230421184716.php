...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  switch ($_POST['action']) {
    // ...
    case 'update_score':
        if (isset($_POST['score_id'], $_POST['category1'], $_POST['category2'], $_POST['category3'], $_POST['category4'])) {
            $score_id = $_POST['score_id'];
            $category1 = $_POST['category1'];
            $category2 = $_POST['category2'];
            $category3 = $_POST['category3'];
            $category4 = $_POST['category4'];
            $result = updateScore($score_id, $category1, $category2, $category3, $category4);
  ...
  if (isset($_POST['action'])) {
      ...
      if ($action == 'add_student') {
        ...
        $student_id = $_POST['student_id'];
            ...
            exit();
      }elseif($action  == 'check_score_exists'){
        if (isset($_POST['student_id'])) {
          $student_id = $_POST['student_id'];
          $scoreExists = checkScoreExists($student_id);
          echo json_encode($scoreExists);
          exit();
        }
      }elseif($action  == 'get_student_scores'){
        if (isset($_POST['student_id'])) {
          $student_id = $_POST['student_id'];
...
function checkScoreExists($student_id) {
  global $conn;
...
function updateScore($score_id, $category1, $category2, $category3, $category4) {
  global $conn;
  $sql = "UPDATE scores SET category1 = ?, category2 = ?, category3 = ?, category4 = ? WHERE id = ?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("iiiii", $category1, $category2, $category3, $category4, $score_id);
  $stmt->execute();
